// Javascript file for myTrips.

// Open new window to display the trips.
function openMyTripsPopup() {
  const popupWindow = window.open(
    "../templates/myTrips.html",
    "My Trips",
...
// Fetch the confirmed trips and display them in window.
async function fetchConfirmedTrips() {
  const customerDetails = await fetchCustomerDetails();
  const response = await fetch(
    "http://localhost:5005/trips/confirmed/customer/" +
